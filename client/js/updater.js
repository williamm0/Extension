...
function manualCheckUpdate() {
    ...
    var includeAlpha = document.getElementById('checkAlphaReleases') && document.getElementById('checkAlphaReleases').checked;
    if (btn) { btn.disabled = true; btn.textContent = 'Checking...'; }
    runUpdateCheck(includeAlpha, function(found) {
...
function runUpdateCheck(includeAlpha, callback) {
    var url = includeAlpha
            ...
            var req = https.get(url, {
                ...
                if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
                    fetchUrlText(res.headers.location, function(err, text) {
                        ...
                        if (callback) callback(processReleaseJson(text, includeAlpha));
                    });
                    return;
                }
                ...
                res.on('end',  function() {
                    if (res.statusCode === 200) {
                        var found = processReleaseJson(body, includeAlpha);
    ...
    function onData(json) {
        var found = processReleaseJson(json, includeAlpha);
        if (callback) callback(found);
    }
...
function processReleaseJson(json, includeAlpha) {
    try {
...
function isNewerRelease(a, b, releaseIsPrerelease) {
    ...
    // Numerically equal: a stable release is newer than a pre-release of the same version
    var currentIsPre = /pre|beta|alpha|rc/i.test(String(b || ''));
    if (currentIsPre && !releaseIsPrerelease) return true;
    return false;
}
...
function getUpdateWorkRoot(os, path) {
    var home = os.homedir ? os.homedir() : '';
    if (home) {
        if (process.platform === 'darwin') return path.join(home, 'Library', 'Application Support', 'jx Tools', 'Updates');
        if (process.platform === 'win32') return path.join(process.env.APPDATA || path.join(home, 'AppData', 'Roaming'), 'jx Tools', 'Updates');
        return path.join(process.env.XDG_DATA_HOME || path.join(home, '.local', 'share'), 'jx-tools', 'updates');
    }
    return path.join(getExtensionPath(), 'update', 'work');
}
...
function installUpdateInPanel(url, tag, done) {
    ...
    var workBase = getUpdateWorkRoot(os, path);
    var tmpRoot = path.join(workBase, 'download_' + Date.now());
    var zipPath = path.join(tmpRoot, 'jx_update.zip');
    var extractDir = path.join(tmpRoot, 'extracted');
    var backupDir = path.join(workBase, 'backup_' + Date.now());
