var graphEditor = (function () {
    ...
    function init(el) {
        ...
        // Wait one frame so CSS has laid out the canvas before measuring
        requestAnimationFrame(function () {
            setupSize();
            redraw();
        });

        // Keep canvas sharp when the panel is resized
    ...
    function fromCanvas(cx, cy) {
        return {
            x: clamp01((cx - M) / (W - 2 * M)),
            y: clamp01(1 - (cy - M) / (H - 2 * M))
        };
    }
    ...
    function redraw() {
        ...
        var accent = getComputedStyle(document.documentElement)
            .getPropertyValue('--accent').trim() || '#c09050';
    ...
    function easeValues() {
        return {
            out: Math.round(h1.x * 100),
            in:  Math.round((1 - h2.x) * 100)
        };
    }

    var PRESETS = {
        linear:    [{ x: 0.33, y: 0.33 }, { x: 0.67, y: 0.67 }],
        ease:      [{ x: 0.25, y: 0.10 }, { x: 0.25, y: 1.00 }],
        easeIn:    [{ x: 0.42, y: 0.00 }, { x: 1.00, y: 1.00 }],
        easeOut:   [{ x: 0.00, y: 0.00 }, { x: 0.58, y: 1.00 }],
        easeInOut: [{ x: 0.42, y: 0.00 }, { x: 0.58, y: 1.00 }]
    };
    ...
    function renderCurveLibrary() {
        ...
        list.innerHTML    = '';
        savedCurves.forEach(function (c) {
            list.appendChild(makeCurveItem(c));
        });
    }
